...
const OTPPage = () => {
  ...
  const isRegisterFlow = location.pathname === "/otp-register";
  const email = isRegisterFlow
    ? registerData?.email
    : localStorage.getItem("emailForOTP");

  useEffect(() => {
    if (!email) {
      navigate(isRegisterFlow ? "/register" : "/login");
    }
  }, [email, navigate, isRegisterFlow]);
  ...
  const getBackLink = () => {
    return isRegisterFlow ? "/register" : "/login";
  };

  const getBackLinkText = () => {
    return isRegisterFlow ? "Back to Registration" : "Back to Login";
  };
  ...
  const getDescription = () => {
    return isRegisterFlow
      ? `We've sent a 4-digit code to ${email}. Please enter the code below to complete your registration. OTP will expire in 5 minutes.`
      : `We've sent a 4-digit code to ${email}. Please enter the code below to continue. OTP will expire in 5 minutes.`;
  };
  ...
  return (
    <div className="h-screen w-full bg-neutral-900 flex flex-col items-center justify-center text-neutral-100">
      <div className="mb-10 flex-col flex items-center w-80">
        ...
        <p className="text-xl font-bold mt-2">{getTitle()}</p>
