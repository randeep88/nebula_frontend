  ...
  return (
    ...
    >
      ...
      >
        ...
        <h1 className="font-[800] text-neutral-100 text-2xl">Top Albums</h1>
      </div>

      {/* Albums Grid */}
      <div className="grid grid-cols-6 px-5 gap-4 pb-5">
        {artistDetails?.topAlbums?.map((album, index) => (
          <Link to={`/album/${album.id}`} key={index}>
            <div className="cursor-pointer transition-all hover:bg-neutral-700/60 p-3 rounded flex flex-col items-center">
              {/* Album Image */}
              <div>
                {album?.image?.[2]?.url ? (
                  <img
                  ...
                  <div className="w-40 h-40 bg-neutral-700 rounded-md animate-pulse mb-3" />
                )}
              </div>
